...
function AdminLogin() {
  ...
  const [bg, setBg] = useState<{ pattern: string | null; floating: string | null }>({
    pattern: null,
    floating: null,
  });
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data)
          setBg({
            pattern: (data as any).login_bg_pattern ?? null,
            floating: (data as any).floating_element_image ?? null,
          });
      });
  }, []);
      ...
      <div className="relative w-full max-w-md overflow-visible rounded-3xl border border-white/40 bg-card/80 p-8 shadow-elegant backdrop-blur-xl">
        ...
        <form onSubmit={handle} className="space-y-3">
          ...
          <Button
            type="submit"
            disabled={busy}
            className="mt-2 h-11 w-full rounded-xl hero-gradient text-base font-bold text-primary-foreground"
          >
